...
class MainWidget(QMainWindow):    
    def __init__(self):
        ...
        self.setWindowTitle('Quantifit version '+str(self.tags['QF']['version']) +'qt serial #: 1')
        self.add_spectrum = []
        self.statusBar()
        self.tabCurrent = 1
        #============================================================================
        ...
        # plot2 = QWidget()
        si_plot = pg.GraphicsLayoutWidget()
        p1 = si_plot.addPlot(row=0, col=0)
        ...
        p2 = si_plot.addPlot(row=0, col=1, colspan=3)
        p2.plot([0,1,2,3,4], [1,6,2,4,3], pen='r')
        self.si_plot = p2
        #p2.setMinimumHeight(800)
        #v = si_plot.addViewBox(row=0, col=0, colspan=4)
        # plot2.setLayout(si_plot)
        self.si_img = pg.ImageItem(np.zeros([2,2]))
          
        p1.addItem(self.si_img)
        ...
        plotLayout2.addWidget(si_plot)        
        self.plot2 = si_plot
        p1.scene().sigMouseClicked.connect(self.mouse_clicked)  
    ...
    def mouse_clicked(self, mouseClickEvent):
        ...
        print(dir(mouseClickEvent))
        print(mouseClickEvent.pos(), mouseClickEvent.currentItem)
    ...
    def save_file(self, filename=None):
        ...
        if extension == 'qfs':
            #Update sesseion file 
            for i in range(10):
                self.tags['QF'][str(i)].update(self.spec[i].tags.update())
                for j in range(tags['QF'][str(i)]['numberOfEdges']):
                    self.tags['QF'][str(i)]['edgeXtags'][str(j)].update(self.spec[i].edgeXsec[j].tags)
            f = open(filename, 'wb')
    ...
    #********************************************
    #******* R E A D Session F I L E ****************
    #********************************************

    def readSession(self, fileName):
        ...
        foreMem = self.tags['QF']['Fore']
        qf = self.tags['QF'][str(foreMem)].tags
        if self.tags['QF']['verbose'] >0:print (qf)#['Calibrations'])
        self.tags['QF'].update(tags['QF'])
        #self.tags['SI'].update(tags['SI'])

        for i in range(10):
            self.spec[i].tags.update(tags['QF'][str(i)])
            self.tags['QF'][str(i)].update(self.spec[i].tags)
            del(self.spec[i].edgeXsec[:])
            for j in range(tags['QF'][str(i)]['numberOfEdges']):
                self.spec[i].edgeXsec.append(EdgeClass(self))
                self.spec[i].edgeXsec[j].tags.update(tags['QF'][str(i)]['edgeXtags'][str(j)])
                self.tags['QF'][str(i)]['edgeXtags'][str(j)] = self.spec[i].edgeXsec[j].tags
        ...
        self.SelectDialog.MemoryAssign(spec,ene,foreMem,1,'Spectrum',qf)
        ...
        if self.main in self.datasets:
            if 'SPECT' in self.datasets[self.main].data_type.name:
                if 'SPECTRAL' in self.datasets[self.main].data_type.name:
                    print('si')
                    self.si_img.setImage(np.array(self.datasets[self.main]).sum(axis=2) )
                    plt= self.si_plot

                    spectrum = np.array(self.datasets[self.main])[0,0]
                else:
                    ...
                    spectrum = np.array(self.datasets[self.main])
                    
                plt.clear()
                ene = np.array(self.datasets[self.main].get_spectral_dims(return_axis=True)[0])
                energy_scale = np.append(ene, ene[-1])
                curve = pg.PlotCurveItem(np.array(energy_scale), spectrum*self.intensity_scale, 
    ...
    def setDefaultValues(self):
        ...
        self.tags['plot']['QF'] = {}
        self.tags['plot']['GF'] = {}
        self.tags['plot']['GF']['Fore'] = 0
        
        self.tags['plot']['GF']['Visible']=vis.copy()
        self.tags['plot']['GF']['Visible']['0'] = 1

        colors = (  'blue', 'red','green', 'orange', 'purple', 'cyan', 'magenta', 'grey', 'lightgrey', 'black','black')
        
        for i in range(10):
            self.tags['plot']['QF'][str(i)]={}
            self.tags['plot']['GF'][str(i)]={}
            self.tags['plot']['QF'][str(i)]['Colors']=QColor(colors[i]).name()
            self.tags['plot']['GF'][str(i)]['Colors']=QColor(colors[i]).name()
            self.tags['plot']['QF'][str(i)]['LineWidth']=1
            self.tags['plot']['GF'][str(i)]['LineWidth']=1
        
        #for i in range(12):
        #    self.tags['QF'][str(i)]=SpectrumQF(self)
        ...
        # read python dict back from the file
        # pkl_file = open(config_path+'\edges_db.pkl', 'rb')
    
        # self.tags['edges_dict'] = pickle.load(pkl_file)
        # pkl_file.close()
